...
def buscar_dados_serie_a():
  ...
  try:
    ...
    if resp_standings.status_code == 200:
      ...
      # Navega na estrutura da ESPN para achar a tabela de classificação
      standings = (
      ...
      for pos, entry in enumerate(standings, start=1):
        ...
        # Pega o logo oficial se disponível
        logos = team_info.get("logos", [])
        logo_url = logos[0].get("href", "") if logos else ""

        # Estatísticas (Pontos, Jogos, Vitórias, Saldo, etc.)
  ...
  try:
    ...
    if resp_score.status_code == 200:
      ...
      for evento in eventos:
        ...
        tipo_status = status_info.get("type", {}).get(
        ...
        if tipo_status == "STATUS_FINAL":
          ...
          try:
            ...
            if agora >= (data_hora_jogo - timedelta(minutes=10)) and agora < (
                data_hora_jogo + timedelta(hours=2)
            ):
              status_final = "ao_vivo"
